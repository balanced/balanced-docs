...
class Spec(dict):
    ...
    def match_form(self, name):
        ...
        if reqs[0]['request']:
            if resource in reqs[0]['request']:
                required = set(reqs[0]['request'][resource][0].keys())
            else:
                required = set(reqs[0]['request'].keys())
        else:
            required = set({})
        ...
        for r in reqs:
            ...
            if r['request']:
                if resource in r['request']:
                    for key, val in r['request'][resource][0].iteritems():
                        if val is None:
                            nullable.add(key)
                else:
        ...
        ignored_keys = \
                    ["bank_accounts_id", "credits_id", "cards_id",
